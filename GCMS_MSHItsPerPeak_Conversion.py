...
path_GCMSMSHitsPerPeak = os.path.join(path_Source, "GCMS_MSHitsPerPeak_Unprocessed.csv")
path_GCMSPeakResults = os.path.join(path_Source, "GCMS_Peak_Results.csv")
...
with open(path_GCMSMSHitsPerPeak, 'r', encoding='UTF-8') as file:
    namesOfHits = []
...
DF_PeakNumberNames.to_csv("GCMS_MSHitsPerPeak.csv",index=False)
...
DF_PeakNumberNamesSorted.to_csv("GCMS_MSHitsPerPeakSorted.csv", index=False)
